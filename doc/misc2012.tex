...
\begin{itemize}
  \item Initialization
    \begin{itemize}
      ...
      \item $\mathrm{init}=
             \begin{array}[t]{@{}l@{}}
             \bigcup_{\mathrm{init}(\pm\mathit{``whatever"}(X,\vec{x}))}\mathrm{init}(\pm\mathit{``whatever"}(X,\vec{x})) 
             \cup {} \\
             \{p\in \mathcal{P}\setminus \mathcal{R} \mid
               q\in \mathrm{init}({-}\mathit{``whatever"}(R,\vec{x})),
               p.\mathrm{name}=q.\mathrm{name}\}
             \end{array}$
    \end{itemize}
  \item Calculation
    \begin{itemize}
